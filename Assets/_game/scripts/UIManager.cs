...
public class UIManager : Singleton<UIManager>
{
    ...
    private void Start()
    {
        if(canvasLoading != null) canvasLoading.gameObject.SetActive(true);
    ...
    {
        RectTransform canvasRectTF = canvasMain.GetComponent<RectTransform>();
        screenWidth = canvasRectTF.sizeDelta.x /*/ canvasRectTF.localScale.x*/;
        screenHeight = canvasRectTF.sizeDelta.y /*/ canvasRectTF.localScale.x*/;
        screenWidth = Screen.width / canvasRectTF.localScale.x;
        screenHeight = Screen.height / canvasRectTF.localScale.x;
        screenWidth = canvasRectTF.rect.width;
        screenHeight = canvasRectTF.rect.height;
    }
    ...
    public T GetUI<T>() where T : UICanvas
    {
        if (!IsLoaded<T>())
        {
            UICanvas canvas = Instantiate(GetUIPrefab<T>(), canvasMain);
    ...
    private T GetUIPrefab<T>() where T : UICanvas
    {
        if (!uiCanvasPrefab.ContainsKey(typeof(T)))
        {
            ...
            {
                uiResources = Resources.LoadAll<UICanvas>("UICanvases");
            }
